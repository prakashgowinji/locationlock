package com.loationlock.data;

/**
 * Created by prakash on 14/09/16.
 */
public class DataConstants {

    public static final String LOCATION_NODE_NAME = "location_lock";
    public static final String LOCATION_NODE_NAME_PATH = "/location_lock/";

    public static final String DEVICE_ID = "deviceId";
    public static final String LOCATION_LATITUDE = "latitude";
    public static final String LOCATION_LONGITUDE = "longitude";
    public static final String LOCATION_ALTITUDE = "altitude";
    public static final String LOCATION_TIME = "timeInmillis";
    public static final String LOCATION_PROVIDER = "provider";
}
